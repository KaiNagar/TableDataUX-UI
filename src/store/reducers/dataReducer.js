...
export function dataReducer(state = INITIAL_STATE, action) {
    ...
    switch (action.type) {
        ...
        case ADD_ROW:
            ...
            return { ...state, data }
        case REMOVE_COLUMN:
            let lastRemovedColumn = state.data.columns.find(c => c.id === action.columnId)
            let newColumns = state.data.columns.filter(c => c.id !== action.columnId)
            return { ...state, data: { ...state.data, columns: newColumns } }
        case REMOVE_ROW:
            let lastRemovedRow = state.data.rows.find(r => r.id === action.rowId)
            let newRows = state.data.rows.filter(r => r.id !== action.rowId)
            return { ...state, data: { ...state.data, rows: newRows } }
        case UPDATE_CELL:
            let lastUpdatedCellValue = state.data.rows[action.info.rowIdx][action.info.columnId]
